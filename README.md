...
Если совсем просто, то проект делает вот что:
...
Если не хочется разбираться во всем репозитории, запомни только это:
...
## Если хочешь просто запустить все по-человечески
...
Если не хочешь заново собирать и размечать данные, можно скачать уже готовый датасет:
...
2. Распакуй его.
3. Помести содержимое в корень проекта `/Users/nikitaisenko/Documents/pore_research_and_ai`.
4. После распаковки в проекте должны появиться или обновиться папки вроде:
   `RealPoresImages/`
   `dataset_manual_prepared/`
   `artifacts/`
5. После этого можно сразу запускать обучение или тестирование без ручной разметки.
...
Если коротко: `mask.png` говорит, где поры вообще есть, а `distance_map.png` помогает модели понять их форму и центр.
...
Если коротко: такой подход удобен, когда поры расположены близко друг к другу и обычная бинарная маска может их слипать.
...
- `precision` — сколько найденных моделью пор действительно являются порами. Высокий precision значит, что модель почти не придумывает лишние поры.
...
- хочешь понять, врет ли модель по общей пористости: смотри `porosity error`
...
- `pore precision = 1.0000`
  Зато почти не придумывает лишние поры.
- `pore F1 = 0.7742`
...
То есть на текущем этапе модель уже очень полезна для оценки общей пористости и общей структуры пор, но для идеального подсчета каждой отдельной поры данных пока маловато.
