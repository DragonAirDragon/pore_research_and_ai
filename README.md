...
Важно: для этого этапа были вручную размечены только 6 реальных изображений пор. Несмотря на такой маленький объем данных, модель уже показала отличный результат на отложенном тесте и на наборе `test_robust`.
